                ...
                {users.map((user, i) => (
                    ...
                    >
                        <div className="w-16 h-16 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-full flex items-center justify-center text-xl font-bold text-white uppercase group-hover:scale-105 transition-transform">
                            {/* Handle potentially missing name fields safely */}
                            {(user.firstName ? user.firstName.charAt(0) : (user.name || user.email || "?").charAt(0))}
                        </div>
                        <div className="flex-1">
                            <h4 className="text-lg font-bold text-white capitalize">
                                {user.firstName && user.lastName ? `${user.firstName} ${user.lastName}` : (user.name || user.email || "Unknown User")}
                            </h4>
                            <p className="text-gray-400 text-sm mb-1">{user.email}</p>
                            <div className="text-gray-500 text-xs mb-2 truncate max-w-[200px] flex flex-col gap-1">
                                <span className="flex items-center gap-1"><Phone size={10} /> {user.phone || "No Phone"}</span>
                                <span className="flex items-center gap-1"><MapPin size={10} /> {user.address || "No Address"}</span>
                            </div>
                            <div className="flex gap-2 flex-wrap">
                                {Array.isArray(subscribers) && subscribers.some(sub => sub.email === user.email) && (
                                    <span className="text-xs bg-purple-500/10 text-purple-400 px-2 py-1 rounded border border-purple-500/20 shadow-sm shadow-purple-500/10 flex items-center gap-1">
                                        <Mail size={10} /> Subscriber
                                    </span>
                                )}
                                <span className="text-xs bg-green-500/10 text-green-400 px-2 py-1 rounded border border-green-500/20 shadow-sm shadow-green-500/10 flex items-center gap-1">
                                    <CreditCard size={10} /> Balance: ₦{(user.balance || 0).toLocaleString()}
                                </span>
                            </div>
                        </div>
                        <button
